...
class SASRec(BasicModel):
    def __init__(self, config):
        ...
        self.SAB = SelfAttentionBlock(config)
        self.dropout_rate = config['sal_dropout']
    ...
    # basic encoding: item embedding + position embedding
    def Encoding(self, input_seq):  # (b, l)
    ...
    def forward(self, inputs):
        (seq_emb, attn_mask) = inputs
        seq_repr = self.SAB(seq_emb, attn_mask)
        output = seq_repr[-1]
...
class Encoder(BasicModel):
    def __init__(self, config):
        ...
        self.dropout_rate = config['lmsal_dropout']
...
class CBAF(BasicModel):
    ...
    def forward(self, query, key, value=None):
        gather_output = self.cba(query=query, key=key, value=value)
        gather_output = self.ffn(gather_output)
        gather_output = self.layernorm(gather_output)
...
class EIDP(BasicModel):
    def __init__(self, config):
        ...
        # self.BehaSpec_Encoder = nn.ModuleList()
        # for _ in range(self.behavior_types):
        #     bs_encoder = SASRec(config=config)
        #     self.BehaSpec_Encoder.append(bs_encoder)

        ## CBAF
        self.cbaf = CBAF(config=config)

        # Implicit Modeling Path (EMP)
        ## PBS-TPE
        ...
        fname = f"{self.model_name}_model.dataset={config['dataset']}." \
                f"lmsal={config['lmsals']}.lh={config['lmsal_heads']}.ld={config['lmsal_dropout']}." \
                f"dd={config['dcba_dropout']}.no={self.no}.pth"
    ...
    def _imp_forward(self, u, input_seqs, bs_seqs, target_bs):
        ...
        # average fusion
        # (b, L, bt, d) --> (b, L, d)
        # X_imp = torch.mean(all_behavior_view_repr, dim=-2)

        # Cross Attention
        # X_imp = self.CBAF(seq_logits, all_behavior_view_repr, all_behavior_view_repr)
        ...
        # # (b, L, bt)
        # batch_u_factors = batch_u_factors.unsqueeze(-2).repeat(1, self.maxlen, 1)
        # batch_u_factors = nn.Softmax(dim=-1)(batch_u_factors)
        # X_imp = torch.einsum('bLt,bLtd->bLd', batch_u_factors, all_behavior_view_repr)

        # target_bs_embs = self.set2emb(u, target_bs)
    ...
    def _emp_forward(self, u, input_seqs, bs_seqs):
        ...
        X_emp = self.pbs_tpe(
            ([u, seq_logits, item_emb, pos_emb, bs_seqs, bs_emb],
             self.attn_mask))  # item+pos+bs 4 attn
    ...
    def forward(self, inputs):
        ...
        X_emp = self._emp_forward(u, input_seqs, bs_seqs)
        X_imp = self._imp_forward(u, input_seqs, bs_seqs, target_bs)
